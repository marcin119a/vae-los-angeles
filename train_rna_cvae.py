"""
Training script for RNACVAE (Conditional VAE for RNA)
"""
import os
import pickle
import numpy as np
...
from src.models import RNACVAE
from src.data import MultiModalDataset
from src.utils.cvae_losses import rna_cvae_loss
...
def train_epoch(model, dataloader, optimizer, epoch):
    ...
    for tpm, beta_data, site in dataloader:
        ...
        # Forward pass: reconstruct RNA conditioned on primary_site and RNA
        recon_rna, mu, logvar = model(rna=tpm, site=site)
        
        # Compute loss
        loss, recon_loss, kld_loss = rna_cvae_loss(
...
def plot_losses(train_losses, val_losses, run_id):
    ...
    plt.title("Training & Validation Loss for RNACVAE")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    filename = f'plots/training_losses_rna_cvae_{run_id}.png'
...
def main():
    ...
    print(f"Starting RNACVAE training run: {run_id}")
    ...
    # Initialize model
    print(f"\nInitializing RNACVAE model on {Config.DEVICE}...")
    model = RNACVAE(
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            # Save best model
            model_path = os.path.join(Config.CHECKPOINT_DIR, f'best_rna_cvae_{run_id}.pt')
    ...
    with open('latest_rna_cvae_run_id.txt', 'w') as f:
        f.write(run_id)
    ...
    print(f"Best model saved to: {os.path.join(Config.CHECKPOINT_DIR, f'best_rna_cvae_{run_id}.pt')}")
    print("="*50)
