"""
Training script for DNACVAE (Conditional VAE for DNA)
"""
import os
import pickle
import numpy as np
...
from src.models import DNACVAE
from src.data import MultiModalDataset
from src.utils.cvae_losses import dna_cvae_loss
...
def train_epoch(model, dataloader, optimizer, epoch):
    ...
    for tpm, beta_data, site in dataloader:
        ...
        # Forward pass: reconstruct DNA conditioned on primary_site and DNA
        recon_dna, mu, logvar = model(dna=beta_data, site=site)
        
        # Compute loss
        loss, recon_loss, kld_loss = dna_cvae_loss(
            recon_dna, beta_data, mu, logvar, beta=beta
        )
...
def plot_losses(train_losses, val_losses, run_id):
    ...
    plt.title("Training & Validation Loss for DNACVAE")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    filename = f'plots/training_losses_dna_cvae_{run_id}.png'
...
def main():
    ...
    print(f"Starting DNACVAE training run: {run_id}")
    ...
    # Initialize model
    print(f"\nInitializing DNACVAE model on {Config.DEVICE}...")
    model = DNACVAE(
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            # Save best model
            model_path = os.path.join(Config.CHECKPOINT_DIR, f'best_dna_cvae_{run_id}.pt')
    ...
    with open('latest_dna_cvae_run_id.txt', 'w') as f:
        f.write(run_id)
    ...
    print(f"Best model saved to: {os.path.join(Config.CHECKPOINT_DIR, f'best_dna_cvae_{run_id}.pt')}")
    print("="*50)
